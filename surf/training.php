...
class trainingbeheer
{
	...
	foreach ($eventList as $key => $val) {
	    if (count($val["calendar"]) == 0) continue;
	...
	foreach ($upcoming as $start => $id) {
	    $participants = $this->eventRegister->getParticipants(
		$id, $start);
	    $names = array();
	    foreach ($participants as $userId) {
		$user = $this->users->get($userId);
		if ($user) array_push($names, 
		    $user["roepnaam"]." ".
		    $user["voorvoegsel"]." ".
		    $user["naam"]);
	    }
	...
	foreach ($this->information as $id) {
	    $result = $this->teksten->readSelect(
		array("id" => $id));
	    if ($result) {
	...
	while ($row = mysql_fetch_assoc($result)) {
	    ...
	    array_push($retval[$lastId]{"calendar"}, $row);
	}
	foreach (array_keys($retval) as $id) {
	    $retval[$id]{"event"} = $this->event->get($id);
	}
	return $retval;
    }
	...
	foreach (array_keys($eventList) as $id) {
	    if (count($eventList[$id]["calendar"])==0) continue;
	    $result[$id] = array();
		...
		if ($start < $userData[$id][$e]["start"])
		{
			...
			$this->makeFormElement($id, $start, 0));
		    $c++;
		} else if ($start == $userData[$id][$e]["start"])
		{
		    array_push($result[$id]["eventRegister"],
	...
	if ($this->userClan) {
		...
		$userId = $usr["id"];
		$userList["$userId"] = $usr["roepnaam"]." ".
		    $usr["voorvoegsel"]." ".$usr["naam"];
	    }
	}
	return $userList;
    }
	...
	$eventList = $this->getEventList(general::TRAINING,
	    date("Y-m-d 00:00")); // start today
	$userDates = $this->getUserDates(date("Y-m-d 00:00"),
	    $currentUserId, array_keys($eventList));
	...
	if ($this->userSession->isLoggedIn()) {
	    ...
	    $this->userClan = $this->users->getClan(
		$this->userSession->getUserAttr("id"));
	} else {
	    $userClan = array();
	}
	...
	if ( $goRegister &&
	    (genGetPageCache("NOROBOT") || general::reCAPTCHAverify())) {
