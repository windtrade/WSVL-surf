...
class htmlforms
{
	...
	if ($pd["protected"] && $pd["value"] != "") {
	    $result .= "<input name=\"$name\" type=\"hidden\" value=\"".
		$pd["value"]."\" />";
	}
    ...
    {
	return "<td>".$pd["label"]."</td><td>".
	    "<!-- unsupported input type: ".$pd["type"]." -->".
	...
	if (strlen($pd["name"]) ==0) {
	    $pd["name"] = sprintf("%s__%3d",
		$pd["type"], $this->fieldID++);
	}
	if (
	    $pd["type"] == "date" ||
	    $pd["type"] == "email" ||
	...
	} else if($pd["type"] == "datetime-local") {
