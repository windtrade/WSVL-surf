...
class news extends table
{
		...
		# in reversed order by publication date (=hotFrom)
	{
		...
		if (0 < mysql_num_rows($result)) return $result;
	...
	# Select all hot News
	{
		...
		$tomorrow->modify("tomorrow");
		$sqlCmd = "SELECT * FROM ".$this->getTableName()." where ".
			"news_hotFrom <= '".$tomorrow->format('Y-m-d')."' and ".
			"news_hotTo >= '".$today->format('Y-m-d')."' ".
			"ORDER BY news_hotTo DESC";
		$result = mysql_query($sqlCmd);
		if (!$result) {
			genSetError("SqlCommand: ".$sqlCmd);
			$this->tbError(NULL);
