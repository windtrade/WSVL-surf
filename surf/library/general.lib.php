...
class general
{
	...
	if (!array_key_exists('loggedIn', $_SESSION))
	    $_SESSION['loggedIn'] = 0;
	$this->action = "http://".$_SERVER['SERVER_NAME'].
	    $_SERVER['PHP_SELF'];
	if (!isset($our_timezone)) $our_timezone="Europe/Amsterdam";
	...
	$this->smarty->registerPlugin('function', 'HFform',
	    array($this->htmlforms, 'HFform'));
	$this->smarty->registerPlugin('function', 'HFlabeledField',
	    array($this->htmlforms, 'HFlabeledField'));
	$this->smarty->registerPlugin('function', 'HFrecaptcha',
	    array($this->htmlforms, 'HFrecaptcha'));
	$this->smarty->registerPlugin('function', 'HEelement',
	    array($this->htmlelements, 'HEelement'));
	$this->smarty->registerPlugin('function', 'HEimage',
	    array($this->htmlelements, 'HEimage'));
	$this->smarty->registerPlugin('function', 'HEtext',
	    array($this->htmlelements, 'HEtext'));
	$this->smarty->registerPlugin('function', 'HEcssmenu',
	    array($this->htmlelements, 'HEcssmenu'));
	$this->smarty->registerPlugin('function', 'HEbuildURI',
	    array($this->htmlelements, 'HEbuildURI'));
	...
	$this->addJavascriptdeclaration("session_lastactive",
	...
	$this->addJavascriptDeclaration(
	    "gLoggedIn", $_SESSION['loggedIn']);
    ...
    {
	array_push($this->javascriptStatements,
	    "var ".$var."= \"".$value."\";");
    }
	...
	if (file_exists($internalScriptName)) {
	    ...
	    $this->setError(
		"$script ($externalScriptName) bestaat niet");
	}
    }
	...
	$curl = curl_init($google."secret=$secret&remoteip=$remoteip".
	    "&response=$response");
...
{
    global $general;
    if (strlen($name)==0) $name = DEFAULT_TEMPLATE;
    if (is_object($general)) {
...
function genCompareClasses($class, $otherClass)
{
	...
	genSetError( "getNumberOfParameters()         : " . $refMethod->getNumberOfParameters() . "\n");
	genSetError( "getNumberOfRequiredParameters() : " . $refMethod->getNumberOfRequiredParameters() . "\n");
	genSetError( "\n");
...
function genFirstDate($wDay, $fMonth, $fDay, $lMonth, $lDay)
{
    ...
    $startYear = 1900 + ($today["tm_mon"] < $lMonth ? $today["tm_year"] : $today["tm_year"]+1);
...
function genNextDate($wDay, $fMonth, $fDay, $lMonth, $lDay)
{
    ...
    if ($firstDate >= $nextDate) return $firstDate;
...
{
    global $general;
    if (!is_numeric($textId) or $textId <= 0) return array();
...
{
    $ok = true;
    if (!array_key_exists("id", $fd["user"])) $fd["user"]["id"] = -1;
    if ($fd["user"]["id"] < 0) {
	// Do not register nothing for an unknown user
	if (count($fd["eventRegister"]) <= 0) return $ok;
    ...
    $newStarts = array();
    foreach (array_keys($eventList) as $key) $newStarts[$key] = array();
    foreach ($fd["eventRegister"] as $key=> $list) {
	// skip registration for nonexistent events
	if (!array_key_exists($key, $fd["eventRegister"])) continue;
	...
	foreach ($this->eventRegister->getEventForUser($evId, $fd["user"]["id"])
	    as $evReg)
	{
		...
		if ($evReg["enrolled"]) {
		    $this->eventRegister->update(
			$evReg, array("enrolled" => 0));
		}
	    } else {
		genSetError(__LINE__.' aanmelden voor '.$evReg["start"]);
		if (!$evReg["enrolled"]) {
		    $this->eventRegister->update(
			$evReg, array("enrolled" => 1));
		}
