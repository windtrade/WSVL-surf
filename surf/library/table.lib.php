...
class table
{
    ...
    private $lastId = 0;
    // supported operators.
    private $operators = array("=", "<", ">", "<=", ">=", "IN", "NOT IN");
	...
	foreach (array_keys($this->structure) as $key) {
	    if (is_array($include) && ! in_array($key, $include)) continue;
	...
	foreach ($this->structure as $key => $struct) {
	    ...
	    if (!array_key_exists($key, $data)) continue;
	    if ($struct["mandatory"] &&
		(!array_key_exists($key, $data) || strlen($data[$key]) == 0)) {
		genSetError("Veld \"".$struct["label"]."\" is niet ingevuld");
		$fieldOK = false;
		if ($struct["mandatory"]) continue;
	    }
	    ...
	    if ($fieldOK &&
		array_key_exists("regexp", $struct) &&
		$struct["regexp"] != "")
	    {
		$regex = "/^".$struct["regexp"]."\$/";
		if (!preg_match($regex, $data[$key])) {
		    $fieldOK = FALSE;
		    if (array_key_exists("msg", $struct) &&
			$struct["msg"] != "") {
			genSetError("Veld '". $struct["label"].
			    "': ".$struct["msg"]);
		    } else {
	...
	if (array_key_exists($name, $data)) {
	    ...
	    $this->setDateTimeLocalToDefault(
		$field["value"], $default);
	} else {
	    $field["value"] = $default;
	}
	...
	if ($nrArgs < 2 || $nrArgs > 4) return $result;
	...
	foreach ($this->structure as $name => $field) {
	    if (is_array($include) && ! in_array($name, $include)) continue;
    ...
    {
	//genDumpVar(__FUNCTION__." arr", $arr);
	$orderByClause = "";
	...
	$cmd = sprintf("select %s %s from %s %s %s",
	    $distinct,
	    $cols,
	    $this->table,
	    $this->makeQueryClause($whereArr),
	...
	return $result;
	 ...
	 if ($query) {
		 ...
		 $this->tbError(get_class($this).":".__FUNCTION__.": ".
		     mysql_num_rows($query)." rijen gevonden ");
	     }
	 } else {
	     $this->tbError(NULL);
	 }
    ...
    {
	if (!property_exists(get_class($this), 'structure')) return $col;
	if (!array_key_exists($col, $this->structure)) return "Onbekend: $col";
	if (strlen($this->structure[$col]["label"])==0) return $col;
