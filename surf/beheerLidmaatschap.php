...
class beheerLidmaatschap
{
	...
	if (array_key_exists(__FILE__, $_SESSION) &&
	    array_key_exists("user", $_SESSION[__FILE__]) &&
	...
	if (array_key_exists('id', $orig[0]['user']) &&
	    array_key_exists($key, $inc) &&
	...
	foreach(array_keys($current["user"]) as $key) {
	    ...
	    array_push($result,
		$this->users->columnToForm(
		    $current["user"], $key, $this->userSession));
	}
	...
	foreach ($fields["userRoles"] as $role) {
		...
		"value" => $role["value"],
		"checked" => (array_key_exists($role["value"],
					       $current["roles"])? 
					       "checked": ""),
		"disabled" => $disabled));
	}
	...
	genSendMail($message, EMAIL_SYSTEM,
	    array($user["email"], $user["emailOuder"]));
	...
	if (!array_key_exists("oldId", $data) ||
	    !array_key_exists("oldWachtwoord", $data) ||
	    !array_key_exists("newWachtwoord", $data) ||
	    !array_key_exists("chkWachtwoord", $data)) {
		genSetError("Incompleet formulier ontvangen");
		return false;
	    }
	if (!$this->testIncomingUserData($data, "oldId")) return false;
	...
	if (!$this->userSession->hasRole(ROLE_SYSTEM)) {
		...
		return password_verify($data["oldWachtwoord"]);
	    }
	}
	return true;
    }
    ...
    {
	if ($this->users->update(array("id" => $data["oldId"]),
	    array("wachtwoord" => $data["newWachtwoord"],
	...
	if (!$this->testIncomingUserData($data, "id")) return false;
	foreach ($fields["user"] as $field => $props) {
	    if (!array_key_exists($field, $data)) continue;
	    if (array_key_exists($field, $old)) {
		$oldData=preg_replace("/[0\-\/]/","", $old[$field]);
		// unchanged data is always right
		if ($oldData == $data[$field]) continue;
	    }
	    if ($props["type"] == "email") {
		$nrEmailFieldsTested++;
		if (!strlen($data[$field])) continue;
		if (!filter_var($data[$field], FILTER_VALIDATE_EMAIL)) {
		    $ok = false; 
		    genSetError("Ongeldig email adres ingevuld bij: ".
			$props["text"]." (".$data[$field].")");
		} else {
		    $nrEmails++;
		}
	...
	if (!$this->users->update($oldData, $data)) { 
		genSetError("De gegevens zijn niet bijgewerkt");
	    }
    }
	...
	foreach ($data as $key => $val) {
	    if (!$this->userRoles->insert(
		array("user_id" => $id, "role" => $val))) {
	...
	if (!array_key_exists("fd", $_REQUEST)) $_REQUEST["fd"] =  array();
	$formData = $_REQUEST["fd"];
	if (!array_key_exists("pw", $formData)) $formData["pw"] = array();
	if (!array_key_exists("user", $formData)) $formData["user"] = array();
	if (!array_key_exists("role", $formData)) $formData["role"] = array();
	if (!array_key_exists("search", $formData)) $formData["search"] = array();

	if (!$this->userSession->isLoggedIn()) {
	    genSetError("Je bent niet (meer) aangemeld, ".
		"meld je eerst opnieuw aan, sorry...");
	} else {
	    if ($action == "updatePassword") {
		if ($this->isValidPassword($formData["pw"])) {
		    $this->updatePassword($formData["pw"]);
		}
		$found = $this->findUsersAndRoles(
		    array("id" => $formData["pw"]["oldId"]));
