    ...
    */
    ...
    private function calendarListToText($list)
    {
        $result = array(
            "event" => $list["event"],
            "GENERAL" => array(),
            "INSTRUCTION" => array("link" => "http://" . $_SERVER["SERVER_NAME"] .
                    "/informatie_leesverder.php?tekst=24&rubriek_id=3&" . "tab=informatie", "dates" =>
                    array()),
            "TRAINING" => array("link" => "http://" . $_SERVER["SERVER_NAME"] .
                    "/training.php?tab=training", "dates" => array()));
        #
            ...
            foreach ($list[$typ] as $eltList) {
                foreach ($eltList as $cal) {
                    if (!array_key_exists($cal["id"], $result[$typ]["dates"])) {
                        $result[$typ]["dates"][$cal["id"]] = array("title" => $list["event"][$cal["id"]]["title"],
                                "start" => array());
                    }
                    $dt = new DateTime($cal["start"]);
                    array_push($result[$typ]["dates"][$cal["id"]]["start"], array("date" => $dt->
                            format("d-m"), "time" => $dt->format("H:i")));
                }
            }
        }
    ...
    {
        $command = $fd["command"];
        if (!in_array($command, array("AANMELDEN", "AFMELDEN")))
            return;
        ...
        $this->eventRegister->register($reg["id"], $reg["start"], $reg["userId"], $reg["enrolled"]);
        $fd["users"] = $us;
    }
    ...
    private function preparePageCache()
    {
        if ($this->userSession->isLoggedIn()) {
            genSetSessionData($_SERVER["PHP_SELF"], "users", $this->users->get($this->
    ...
    public function __construct()
    {
        ...
        $data = array("event" => array(), "calendar" => array());
        ...
        if (genGetPageCache("users", "id")) {
            $reg = $this->eventRegister->get($currentEventItem["id"], $currentEventItem["start"],
        ...
        genSmartyAssign("monthNames", array(
            ...
            "december"));
        // Add Open Graph data
        $event = $data["event"][$currentEventItem["id"]];
        $title = trim($event["title"] . " - " . $currentEventItem["name"], " -");
        $description = $event["text"] . " " . $event["detail"];
