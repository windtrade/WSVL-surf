...
class informatie
{
    ...
    private $userColumns = array("roepnaam", "naam", "voorvoegsel",
	"telefoonnr", "gebdatum", "email");
	...
	if (strtolower($tab) == "surfpool")
	    $nrs = array(22, 28);
	else if (strtolower($tab) == "surfles") {
	    $nrs = array(24, 28);
	    $data = $this->getData(general::INSTRUCTION);
	}
	else if (strtolower($tab) == "wedstrijden")
	    $nrs = array(21, 28);
	else if (strtolower($tab) == "gww")
	    $nrs = array(21, 28);
	$teksten = array();
	foreach ($nrs as $nr) {
    ...
    {
	if (!array_key_exists('fd', $_POST)) return false;
	$fd = $_POST["fd"];
	...
	genDumpVar('$starts', $starts);
	// make up the mail message
	$fields = $fd["user"];
	$fields["start"] =
	    implode(", ", $starts); //comma separated list of selected dates
	...
	$newStarts = array();
	foreach (array_keys($this->eventList) as $key) $newStarts[$key] = array();
    ...
    {
	if (!is_array($fd)) $fd = array();
	if (!array_key_exists("user", $fd)) $fd["user"] = array();
	if (!array_key_exists("eventRegister", $fd)) $fd["eventRegister"] = array();
	$result = array(
	    "user" => array(),
	    "eventRegister" => array());
	$result["user"] = $this->users->toForm(
	    $fd["user"],
	    $this->userSession, 
	    array(), // hide these
	...
	foreach ($this->eventList as $id => $data)
	{
		...
		array_push ($result["eventRegister"][$id],
		    $this->makeFormElement($id, $calendar["start"], $enrolled));
	    }
	}
	return $result;
    }
    ...
    {
	$this->makeObjects();
	$this->eventList = $this->getEventList(general::INSTRUCTION,
	    date("Y-01-01 00:00"));
