...
class aanmeldenLid
{
    ...
    private function sendAlert($action, $data)
    {
        ...
        if (strlen($user['emailOuder']) > 0 && $user["emailOuder"] != $user["email"]) {
    ...
    {
        $data["user"] = $this->users->toForm($fd["user"], $this->userSession, array("wachtwoord" =>
                "hide"));
        $data["interest"] = array();
        foreach ($this->interests as $field => $label) {
            ...
            $data["interest"][$field]["value"] = (array_key_exists($field, $fd["interest"]) ?
                $fd["interest"][$field] : "");
        }
        $data["contact"]["id"] = $this->contact->columnToForm($fd["contact"], "id", $this->
            userSession);
        $data["contact"]["message"] = $this->contact->columnToForm($fd["contact"],
            "message", $this->userSession);
        return $data;
    }
    ...
    private function insert(&$fd)
    {
        if (!$this->users->insert($fd["user"])) {
            genLogVar('$this->users->insert($fd["user"])', false);
            return;
        }
        ...
        if ((genGetPageCache("NOROBOT") || general::reCAPTCHAverify())) {
            ...
            if ($command == "AANMELDEN") {
                if ($this->users->isValid($fd["user"])) {
                    genLogVar("Aanmelding ok=", true);
                    $this->insert($fd);
                    genSetInfo("Hartelijk bedankt voor je aanmelding, hieronder " .
                        "kun je jouw gegevens zonodig nog bijwerken");
                    $command = "BIJWERKEN";
                } else {
                    genLogVar("Aanmelding ok=", false);
                }
            } else
                if ($command == "BIJWERKEN") {
                    if ($this->users->isValid($fd["user"])) {
                        genLogVar("Aanmelding ok=", true);
